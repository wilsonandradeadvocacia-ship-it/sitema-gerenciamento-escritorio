...
import { TABELAS_PESQUISADAS_RAW } from './tabelasHonorariosPesquisadas.js'
...
export interface TabelaHonorariosSeed {
  uf: string
...
function montarGenerica(uf: string, seccional: string): TabelaHonorariosSeed {
  return {
    uf,
...
function montarTabelaPesquisada(raw: (typeof TABELAS_PESQUISADAS_RAW)[number]): TabelaHonorariosSeed {
  ...
  const statusDados: TabelaHonorariosSeed['statusDados'] = raw.uf === 'SP' ? 'pesquisado' : 'parcial'
...
export function buildTabelasParaNovoEscritorio(): TabelaHonorariosSeed[] {
  return UFS.map((uf) => {
    const pesquisada = TABELAS_PESQUISADAS_RAW.find((r) => r.uf === uf.sigla)
    return pesquisada ? montarTabelaPesquisada(pesquisada) : montarGenerica(uf.sigla, uf.seccional)
  })
}
